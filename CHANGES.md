...
- `plugins/pstack/models.json` gains a `providers` list with a single-role and a strongest-role default per provider, a `provider` field on every available model, a `mode` (`consult` or `execute`) on every role, and the Codex models as available entries. The default panel mixes providers (`claude-opus-5-5`, `gpt-5.6-sol`, `claude-fable-5-1`); single-model roles stay on the host provider on both hosts.
- The generator renders every model entry as `slug@provider`, stamps a Roles table and a Providers table into the new `run-role` skill, groups the setup skill's available-model line by provider, and derives the Codex model-names section from the provider defaults and the panel skills. The stray-slug scan covers every slug in `models.json`, not only `claude-*`, and normalises path separators so it runs on Windows.
- New `run-role` skill: the one place that turns a role into running agents. The caller hands it dispatches (role, entry, brief); an entry whose provider is the host is a native subagent, any other entry is a supervised Orca worker dispatched through the bundled `orca-delegate` skill in the role's mode, with `result.md` in the contract's call directory as the default result location. A failed Orca start leaves the other dispatches running, is reported with the attempted command and the error, and the user chooses retry, native substitute, or dropout; nothing falls back on its own. No Orca worker in the current worktree, `consult` included, runs while the caller edits it.
