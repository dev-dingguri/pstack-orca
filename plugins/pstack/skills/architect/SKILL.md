...
Design before implementing. Sketch types, function signatures, class shapes, and module boundaries with `not implemented` bodies and pseudocode. Synthesize across one candidate per provider, then fill in code against the chosen sketch. If implementation proves the sketch wrong, throw it out and redesign.
...
Use your configured architect runners (defaults in [Models](#models)). Tell Arena to resolve its runners through the **run-role** skill with role `architect runners` and the task difficulty, so each candidate runs under that role's entries and mode and writes only its design package. A design that is hard to reverse (a schema or data migration, a public API or config format, a module rewrite) is `strongest`; run-role then swaps each panel entry for its provider's strongest-role default.
