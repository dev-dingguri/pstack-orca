...
- Code crossing a function boundary → the **architect** skill, one design followed by independent cross-provider review before implementing.
- Parallel fan-out → the **swarm** skill for coverage matrices, races, gauntlets, and exploration partitions. Use **arena** only for an explicit candidate-comparison request, with base selection and grafting.
...
Carry the active mode into native briefs and cross-provider contracts through [run-role's propagation policy](../run-role/SKILL.md#carry-poteto-mode-into-the-brief). Implementation workers follow this skill in full. Dedicated review and investigation roles keep their own workflow.
